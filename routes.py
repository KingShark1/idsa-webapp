...
async def add_swimmer(
    ...
    try:
        ...
        if not club:
            ...
            try:
                club_data = ClubModel(
                    name=swimmer_data.club, total_points=0
                )  # Assuming total_points starts at 0
                ...
                if "club" in result:
                    ...
                    raise HTTPException(
                        status_code=400,
                        detail="Something went wrong in adding the new club",
                    )
        ...
        for eid in swimmer_data.event_id:
            ...
            if "Relay" in event.name:
                ...
                relay_event = (
                    db.query(RelayEvent)
                    .filter(
                        RelayEvent.club_id == club.id,
                        RelayEvent.event_id == eid,
                    )
                    .first()
                )
                if not relay_event:
                    participant_count = (
                        db.query(RelayEvent)
                        .filter(RelayEvent.event_id == event.id)
                        .distinct(RelayEvent.club_id)
                        .count()
                    )
                ...
                participant_count = (
                    db.query(SwimmerEvent)
                    .filter(SwimmerEvent.event_id == eid)
                    .count()
                )
                next_lane_index = participant_count % 8
                next_lane_id = lane_order[next_lane_index]
        ...
        logging.error(
            f"Error adding swimmer: {str(traceback.print_exception(e))}"
        )
        raise HTTPException(status_code=500, detail=str(e))
...
def get_events(
    ...
    if gender:
        events = (
            db.query(Event)
            .filter(Event.age_group == age_group)
            .filter(Event.gender == gender.lower())
            .order_by(Event.name)
            .all()
        )
    else:
    ...
    sorted_events = sorted(
        events,
        key=lambda e: e.name.split("mt. ")[-1].strip().lower(),
        reverse=True,
    )
...
def get_swimmers(
    ...
    if event:
        query = (
            query.join(SwimmerEvent)
            .join(Event)
            .filter(Event.name.ilike(f"%{event}%"))
        )
...
async def update_swimmer(
    swimmer_id: int,
    update_data: SwimmerUpdateModel,
    db: Session = Depends(get_db),
):
    ...
    if (
        swimmer.gender != update_data.gender
        or swimmer.age_group != update_data.age_group
    ):
        await delete_swimmer(
            swimmer_id, db
        )  # Reuse the delete swimmer function
...
async def delete_swimmer(swimmer_id: int, db: Session = Depends(get_db)):
    ...
    swimmer_events = (
        db.query(SwimmerEvent)
        .filter(SwimmerEvent.swimmer_id == swimmer_id)
        .all()
    )
    event_ids = {se.event_id for se in swimmer_events}

    # Delete the swimmer's events
    db.query(SwimmerEvent).filter(
        SwimmerEvent.swimmer_id == swimmer_id
    ).delete()
...
async def view_swimmers(
    ...
    if event_name:
        query = (
            query.join(SwimmerEvent)
            .join(Event)
            .filter(Event.name.ilike(f"%{event_name}%"))
        )
    ...
    return templates.TemplateResponse(
        "view_swimmers.html",
        {"request": request, "swimmers": swimmers, "events": events},
    )


@router.get(
    "/swimmers/{swimmer_id}", response_model=SwimmerEventForCompetitionModel
)
...
async def delete_swimmer_event(
    swimmer_id: int, event_id: int, db: Session = Depends(get_db)
):
    # Fetch the swimmer event
    swimmer_event = (
        db.query(SwimmerEvent).filter(SwimmerEvent.id == event_id).first()
    )
    # Check if the swimmer event exists
    if not swimmer_event:
        raise HTTPException(
            status_code=404, detail="Event not found for swimmer"
        )
...
def delete_relay_event(
    swimmer_id: int, relay_event_id: int, db: Session = Depends(get_db)
):
...
async def competition(request: Request, db: Session = Depends(get_db)):
...
async def competition_data(db: Session = Depends(get_db)):
    ...
    for event in events:
        if "Relay" in event.name:
            ...
            participant_count = (
                db.query(SwimmerEvent)
                .filter(SwimmerEvent.event_id == event.id)
                .count()
            )
        event_data.append(
            {
...
async def assign_swimmer_to_event(
    data: AssignSwimmerRequest, db: Session = Depends(get_db)
):
    ...
    if existing_participation:
        raise HTTPException(
            status_code=400,
            detail="Swimmer is already participating in this event",
        )
...
@router.get(
    "/event_participants",
    response_model=Union[List[SwimmerEventModel], List[RelayParticipantModel]],
)
async def get_event_participants(event_id: int, db: Session = Depends(get_db)):
    ...
    if "Relay" in event.name:
        ...
        relay_events = (
            db.query(RelayEvent).filter(RelayEvent.event_id == event_id).all()
        )
        ...
        participants = (
            ...
            .order_by(SwimmerEvent.heat_id, SwimmerEvent.lane_id)
            .all()
        )
        return participants
...
@router.get(
    "/top_8_participants",
    response_model=List[Union[SwimmerEventModel, RelayParticipantModel]],
)
async def get_top_8_participants(event_id: int, db: Session = Depends(get_db)):
    try:
        ...
        participants = (
            ...
            .filter(SwimmerEvent.event_id == event_id)
            .order_by(
                SwimmerEvent.time.asc()
            )  # Sort by time in ascending order (best to worst)
...
async def update_lane(request: UpdateLaneOrder, db: Session = Depends(get_db)):
    swimmer_event = (
        db.query(SwimmerEvent)
        .filter(SwimmerEvent.id == int(request.swimmer_event_id))
        .first()
    )
    if not swimmer_event:
...
async def update_times(
    request: List[Union[UpdateSwimmerTimeRequest, UpdateRelayTimeRequest]],
    db: Session = Depends(get_db),
):
    try:
        for update in request:
            ...
            if isinstance(update, UpdateRelayTimeRequest):
                ...
                relay_event = (
                    db.query(RelayEvent)
                    .filter(RelayEvent.id == update.relay_event_id)
                    .first()
                )
                if relay_event:
                    if update.time == "::":
                    ...
                    logging.info(
                        f"Updated RelayEvent: {relay_event.time}, Club: {relay_event.club.name}, Event ID: {relay_event.event_id}"
                    )
            else:
                # Update time for a regular swimmer event
                swimmer_event = (
                    db.query(SwimmerEvent)
                    .filter(SwimmerEvent.id == update.swimmer_event_id)
                    .first()
                )
                if swimmer_event:
                    if update.time == "::":
                    ...
                    logging.info(
                        f"Updated SwimmerEvent: {swimmer_event.time}, Swimmer: {swimmer_event.swimmer.name}, Event ID: {swimmer_event.event_id}"
                    )
...
async def update_final_times(
    request: List[UpdateFinalTimeRequest], db: Session = Depends(get_db)
):
    try:
        for update in request:
            logging.info(update)
            ...
            if event and event.time_trial is False:
                ...
                final_event = (
                    db.query(FinalEvent)
                    .filter(
                        FinalEvent.event_id == update.event_id,
                        FinalEvent.swimmer_id == swimmer.id,
                    )
                    .first()
                )
                if final_event:
                    ...
                    logging.info(
                        f"Updated FinalEvent: {final_event.time}, Swimmer: {final_event.swimmer.name}, Event ID: {final_event.event_id}"
                    )
                else:
                    ...
                    swimmer_event = db.query(SwimmerEvent).get(
                        update.swimmer_event_id
                    )
                    swimmer = swimmer_event.swimmer
                    ...
                    db.add(final_event)
                    logging.info(
                        f"Created FinalEvent: {final_event.time}, Swimmer: {final_event.swimmer.name}, Event ID: {final_event.event_id}"
                    )

        db.commit()
        return JSONResponse(
            content={"message": "Final times updated successfully"}
        )
    except Exception as e:
...
async def recalculate_heats_lanes(event_id: int, db: Session = Depends(get_db)):
    ...
    participants = (
        db.query(SwimmerEvent).filter(SwimmerEvent.event_id == event_id).all()
    )
    if participants:
...
async def get_event_results(request: Request, db: Session = Depends(get_db)):
    ...
    for event in events:
        if "Relay" in event.name:
            ...
            participants.sort(
                key=lambda se: (
                    convert_time_to_total_ms(se, se.time),
                    se.time is None,
                )
            )
            relay_participants = []
            ...
            if len(participants) > 2:
                if (
                    participants[2].time != "99:99:99"
                    or participant[2].time != "88:88:88"
                ):
                    for se in participants:
                        ...
                        if (
                            se.time is not None
                            and f"{se.medal}" in config["relay_medal_points"]
                        ):
                            # Find the Club
                            # Add point to the club
                            club.total_points += config["relay_medal_points"][
                                f"{se.medal}"
                            ]
                            db.commit()
            for participant in participants:
                ...
                swimmer_names = [
                    swimmer.name for swimmer in participant.swimmers
                ]
                relay_participants.append(
                    {
                        "id": participant.id,
            ...
            if event.time_trial:
                ...
                participants = (
                    db.query(FinalEvent)
                    .filter(FinalEvent.event_id == event.id)
                    .all()
                )
            ...
            if len(participants) > 2:
                ...
                if (
                    participants[2].time != "99:99:99"
                    or participants[2].time != "88:88:88"
                ):
                    for se in participants:
                        # Assign points based on medal
                        if (
                            se.time is not None
                            and f"{se.medal}" in config["medal_points"]
                        ):
                            try:
                                swimmer_points[
                                ...
                                se.swimmer.club.total_points += config[
                                    "medal_points"
                                ][f"{se.medal}"]
                            except Exception as e:
                                logging.error(
    ...
    sorted_championship_standings = {
        ...
        for key in sorted(
            championship_standings.keys(), key=lambda x: (x[0], x[1])
        )
    }

    sorted_club_standings = (
        db.query(Club).order_by(Club.total_points.desc()).all()
    )
    ...
    return templates.TemplateResponse(
        "results.html",
        {
            ...
            "standings": sorted_championship_standings,
...
def create_club(club_data: ClubModel, db: Session = Depends(get_db)):
    try:
        ...
        new_club = Club(
            name=club_data.name, total_points=club_data.total_points
        )
        db.add(new_club)
        db.commit()
        db.refresh(new_club)
