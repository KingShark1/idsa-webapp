document.addEventListener("DOMContentLoaded", () => {
  ...
  function prepareFinalEvent(event, eventTitle) {
    ...
    drawEmptyFinal(event.id, eventTitle); // Draw the empty final heat table
    loadFinalParticipants(event.id); // Load the top 8 participants
  }

  function editFinalEvent(eventId) {
    const finalParticipantsDiv = document.getElementById(
      `final_${eventId}_participants`,
    );
  ...
  function submitFinalEvent(eventId) {
    ...
    const rows = finalParticipantsDiv.querySelectorAll(
  ...
  function drawEmptyFinal(eventId, eventTitle) {
    const finalParticipantsDiv = document.getElementById(
      `final_${eventId}_participants`,
    );
    ...
    finalTable.innerHTML = `
            <thead>
                <tr>
                    <th colspan="7">Final  Start List ${eventTitle}</th>
                </tr>
                <tr>
  ...
  function loadFinalParticipants(eventId) {
    fetch(`/top_8_participants?event_id=${eventId}`)
      ...
      .catch((error) =>
        console.error("Error loading final participants:", error),
      );
  }
