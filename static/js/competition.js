document.addEventListener('DOMContentLoaded', () => {
    ...
    const printNormalEventsButton = document.getElementById('printNormalEventsButton');
    ...
    const printFinalEventsButton = document.getElementById('printFinalEventsButton');
    ...
    function fetchEvents() {
        ...
        fetch('/competition_data')
            ...
            .then(events => {
                const filteredEvents = events.filter(event => event.id >= startId && event.id <= endId);
            ...
            .then(participants => {
                participants.forEach(participant => {
                    ...
                    if (Array.isArray(participant.swimmers)) {
                        ...
                        row.setAttribute('data-participant-id', `relay-${participant.heat_id}-${participant.lane_id}`);
                        row.cells[1].innerHTML = participant.swimmers.map(swimmer => `<div>${swimmer}</div>`).join(''); // Display each swimmer in a new line
    ...
    function submitEvent(eventId) {
        ...
        const updates = [];
        const eventName = document.querySelector(`#event_${eventId}_container h3`).textContent;
    ...
    function openSwimmerDropdown(laneIndex, eventId, heatId) {
        ...
        fetch(`/eligible_swimmers?event_id=${eventId}`)  // Modify this to fetch the correct swimmers based on eventId
            ...
            .then(swimmers => {
                ...
                const row = document.querySelector(`#heat_${eventId}_${heatId} tr:nth-child(${laneIndex})`);
                row.cells[1].innerHTML = '';  // Clear the plus button
                row.cells[1].appendChild(dropdown);  // Insert the dropdown
            })
            .catch(error => console.error('Error fetching eligible swimmers:', error));
    }
    ...
    function assignSwimmerToHeat(swimmerId, laneIndex, eventId, heatId) {
        ...
        fetch(`/swimmers/${swimmerId}`)
            ...
            .then(swimmer => {
                ...
                row.cells[2].textContent = new Date(swimmer.dob).toLocaleDateString('en-GB');
                ...
                .catch(error => console.error('Error assigning swimmer to heat:', error));
            });
    }
